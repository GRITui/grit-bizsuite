...
//
// Construction is LAZY (first property access) rather than at module load,
// so `next build` succeeds with no DATABASE_URL set — importing a route
// module during page-data collection must not throw. The missing-env error
// now surfaces on the first actual query instead.
